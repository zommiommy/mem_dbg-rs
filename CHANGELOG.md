...
- Clarified that `SizeFlags::CAPACITY` is a no-op only for
  `AliasableVec`/`AliasableString`, not for `maybe-dangling`.
- Documented that manual `MemSize::mem_size_rec` implementations must
  return at least `core::mem::size_of_val(self)`.
- The code generated by `#[derive(MemDbg)]` builds its field-offset table
  on the stack instead of allocating a temporary vector per traversal, as
  the field count is a compile-time constant.
